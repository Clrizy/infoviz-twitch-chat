let chat_dataset;
let textYscale;
d3.csv('data/chat.csv').then(data => {
    chat_dataset = data
...
function drawInitial() {
    ...
    let explainer = svg.append('text').attr('fill', 'black').attr('x', 10).attr('y', 300)
...
function drawfunction(index){
    ...
    svg.selectAll('text')
        ...
        .attr('opacity', function(d, i) {
            return (i == index) ? 1.0 : ((i < index) && (i > (index - 8)) ? 1 - (index-i)*0.18 : 0)
        })
        .attr('y', function(d, i) {
            return (i - index)*19 + 160
        });

    svg.select('.big_text').attr('fill', 'black').attr('x', 10).attr('y', 200).attr('opacity', 1.0).text('Chat Log:')
